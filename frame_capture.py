...
class Camera_Thread:
    ...
    # IMPORTANT: queue lebih efisien daripada deque
    # versi queue berjalan pada 35% dari 1 prosesor
    ...
    # Tetapkan nomor sumber kamera (standarnya adalah kamera 0).
    # Atur lebar dan tinggi piksel kamera (standarnya adalah 640x480).
    # Tetapkan frekuensi gambar target (maks) (standarnya adalah 30).
    # Tetapkan jumlah bingkai yang akan disimpan dalam buffer (standarnya adalah 4).
    # Tetapkan variabel buffer_all: True = tidak ada frame loss, untuk membaca file, jangan baca frame lain sampai buffer memungkinkan
    #                               False = memungkinkan kehilangan bingkai, untuk membaca kamera, simpan saja pembacaan bingkai terbaru

    # Mulai camera thread menggunakan self.start().

    # Dapatkan frame berikutnya dengan menggunakan self.next(black=True,wait=1).
    #   Jika hitam, nilai bingkai default adalah bingkai hitam.
    #   Jika tidak hitam, nilai bingkai default adalah None.
    #   Jika timeout, tunggu hingga timeout detik untuk memuat frame ke dalam buffer.
    #   Jika tidak ada bingkai dalam buffer, kembalikan nilai bingkai default.
